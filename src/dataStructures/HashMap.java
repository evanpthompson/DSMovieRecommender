...
public class HashMap<K,V> implements Map<K,V>
{
	...
	private void rehash()
	{
		...
		// create new table
		table = new HashMap.HashMapEntry[newSize];
	...
	public void clear()
	{
		...
		table = new HashMap.HashMapEntry[tableSizes[0]];
		...
		for (int i = 0; i < table.length; i++)
		{
			...
			for (HashMapEntry he = table[i]; he != null; he = he.nextEntry)
			{
				if (he.nextEntry == null)
					result += "(" + he.key.toString()+ " , " + he.value.toString()+ ")";
				else
					result += "(" + he.key.toString()+ " , " + he.value.toString()+ ")" + " --> ";
			}
	 ...
	 */
	protected class HashMapIterator implements MapIterator<K,V>
	{
	...
	protected class HashMapEntry
	{
		 ...
		 */
		HashMapEntry(K itemKey, V itemValue, int hash)
		{
			key = itemKey;
			value = itemValue;
			hashValue = hash;
		 ...
		 */
		public boolean equals(HashMapEntry otherEntry)
		{
			
			
			// create new HashMapEntry
			HashMapEntry he = otherEntry;
