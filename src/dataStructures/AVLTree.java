...
public class AVLTree<T extends Comparable<? super T>>  implements BinarySearchTree<T>
{
	 ...
	 */
	private AVLNode<T> caseOneRotation(AVLNode<T> k2)
	{
	 ...
	 */
	private AVLNode<T> caseTwoRotation(AVLNode<T> k3)
	{
	 ...
	 */
	private AVLNode<T> caseThreeRotation(AVLNode<T> k3)
	{
	 ...
	 */
	private AVLNode<T> caseFourRotation(AVLNode<T> k2)
	{
	 ...
	 */
	protected AVLNode<T> getMinNode(AVLNode<T> node)
    {
		if(node != null)
	 ...
	 */
	protected AVLNode<T> getMaxNode(AVLNode<T> node)
    {
		if(node != null)
     ...
     */
    protected AVLNode<T> add(T obj, AVLNode<T> node)
    {
		if(node == null)
			node = new AVLNode<T>(obj);
	 ...
	 */
	protected AVLNode<T> remove(T obj, AVLNode<T> node)
    {
		if(node == null)
     ...
     */
    protected AVLNode<T> removeMinNode(AVLNode<T> node)
    {
		if(node == null )
			throw new NoSuchElementException("Item does not exist");
     ...
     *	throws NoSuchElementException if the item does not exist
     */
    protected AVLNode<T> removeMaxNode(AVLNode<T> node)
    {
		if(node == null )
			throw new NoSuchElementException("Item does not exist");
	 ...
	 */
	private String toString(AVLNode<T> node, int level)
	{
		String s = "";
	 ...
	 */
	protected abstract class AVLTreeIterator implements OrderedIterator<T>
	{
		 ...
		 */
		public AVLTreeIterator(AVLTree<T> tree)
		{
 			theTree = tree;
		...
		/**********
 		 ...
 		 */
 		protected abstract AVLNode<T> getCurrentNode();
	...
	protected class PreOrderIterator extends AVLTreeIterator
	{
		 ...
		 */
		public PreOrderIterator(AVLTree<T> tree)
		{
 			super(tree);
 			
 			// initialize stack
 			s = new ArrayStack<AVLNode<T>>();
 		 ...
 		 */
 		protected AVLNode<T> getCurrentNode()
 		{
 			if (s.isEmpty())
 				return null;
	...
	protected class PostOrderIterator extends AVLTreeIterator
	{
		 ...
		 *	iteration begins with the root node
		 */
		public PostOrderIterator(AVLTree<T> tree)
		{
 			super(tree);
	...
	protected class InOrderIterator extends PostOrderIterator
	{
		 ...
		 *	iteration begins with the root node
		 */
		public InOrderIterator(AVLTree<T> tree)
		{
 			super(tree);
	...
	protected class LevelOrderIterator extends AVLTreeIterator
	{
		...
		/** stores nodes yet to be traversed */
		private LinkedQueue<AVLNode<T>> q;
		 ...
		 *	iteration begins with the root node
		 */
		public LevelOrderIterator(AVLTree<T> tree)
		{
 			super(tree);

			// initialize queue
 			q = new LinkedQueue<AVLNode<T>>();
	...
	private static class AVLNode<E extends Comparable<? super E>>
	{
		 ...
		 */
		public AVLNode(E obj, AVLNode<E> leftChild, AVLNode<E> rightChild)
		{
			theItem = obj;
			left = leftChild;
			right = rightChild;
